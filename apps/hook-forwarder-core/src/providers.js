...
function clientType(value) {
    ...
    if (raw.includes("vscode") || raw.includes("vs-code"))
        return "vscode";
    if (raw.includes("desktop"))
        return "desktop";
    if (raw.includes("cli") || raw.includes("terminal") || raw.includes("code"))
        return "cli";
    return "unknown";
}
function labelForClient(client) {
    if (client === "cli")
        return "Claude CLI";
    if (client === "desktop")
...
function claudeToolName(payload) {
    ...
    if (CLAUDE_KNOWN_TOOLS.includes(raw)) {
        if (raw === "TaskCreate" || raw === "TaskUpdate")
            return "Task";
        return raw;
    }
...
function claudeHookName(payload) {
    return text(payload.hook_event_name) ?? text(payload.hookEventName) ?? text(payload.event) ?? "Unknown";
}
function claudeDetailForTool(payload, tool, privacyMode) {
    if (privacyMode === "safe")
        return undefined;
    const input = asObject(payload.tool_input);
    if (tool === "Read" || tool === "Edit" || tool === "Write" || tool === "Notebook")
        return basename(text(input.file_path) ?? text(input.path));
    if (tool === "Grep")
        return text(input.pattern) ? "pattern: " + text(input.pattern) : undefined;
    if (tool === "Glob")
        return text(input.pattern) ? "pattern: " + text(input.pattern) : undefined;
    if (tool === "WebSearch")
        return text(input.query) ? "query: " + text(input.query) : undefined;
    if (tool === "Bash")
        return privacyMode === "detailed" ? summarizeCommand(text(input.command)) : undefined;
    if (tool === "Agent") {
        const prompt = text(input.prompt);
        return prompt ? (prompt.length > 40 ? prompt.slice(0, 37) + "..." : prompt) : undefined;
    }
    if (tool === "Skill")
        return text(input.skill) ?? text(input.name);
...
function claudeTitleForTool(tool) {
    if (tool === "Read" || tool === "Notebook")
        return "正在读文件";
    if (tool === "Edit" || tool === "Write")
        return "正在编辑代码";
    if (tool === "Bash")
        return "正在执行命令";
    if (tool === "Grep" || tool === "Glob" || tool === "WebFetch")
        return "正在搜索";
    if (tool === "WebSearch")
        return "正在搜索网络";
    if (tool === "Agent")
...
function claudeClientFromPayload(payload, env) {
    ...
    if (detected !== "unknown")
        return { clientType: detected, clientLabel: labelForClient(detected) };
...
function codexHookName(payload) {
...
function codexTitleForTool(tool) {
    if (tool === "Shell")
        return "正在执行命令";
    if (tool === "UpdatePlan")
        return "正在更新计划";
...
function codexDetailForTool(payload, tool, privacyMode) {
    if (privacyMode === "safe")
        return undefined;
    const input = asObject(payload["tool_input"]);
    if (tool === "Shell")
        return privacyMode === "detailed" ? summarizeCommand(text(input["command"])) : undefined;
    if (tool === "ApplyPatch")
        return text(input["patch"]) ? summarizeCommand(text(input["patch"])) : undefined;
    if (tool === "ViewImage")
        return text(input["path"]) ?? text(input["url"]);
    if (tool === "UpdatePlan")
