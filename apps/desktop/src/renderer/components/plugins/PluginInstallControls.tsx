...
export function PluginInstallControls({ marketItem, installed, installing, onInstall, onRemove, onRunNow, zh = false }: { marketItem?: PluginMarketItem; installed?: CustomPlugin; installing?: boolean; onInstall?: () => void; onRemove?: () => void; onRunNow?: () => void; zh?: boolean }) {
  return (
    <div className="plugin-action-row">
      ...
      {installed && onRunNow && (installed.manifest?.events?.length ?? 0) > 0 ? (
        <button className="ghost-btn plugin-run-now-btn" onClick={onRunNow}>{zh ? "立即运行" : "Run now"}</button>
      ) : null}
